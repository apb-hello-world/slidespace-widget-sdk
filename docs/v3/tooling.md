# Tooling, tests, and performance

The CLI supports `init`, `validate`, `simulate`, `inspect`, `test`, `pack`, `publish`, and `schemas`. Use mocked data and permission failures during local simulation. Validate every package before submission and pin Wasm source metadata to the exact build commit.
